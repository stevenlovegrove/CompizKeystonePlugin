...
void
KeystoneScreen::clearCairoLayer (cairo_t *cr)
{
    cairo_save (cr);
...
void
KeystoneScreen::drawSwitcherBackground ()
{
    cairo_t         *cr;
...
void
KeystoneScreen::setupCairoContext (KeystoneCairoContext &context)
{
    ...
    {
	screen->logMessage ("keystone", CompLogLevelError,
...
void
KeystoneScreen::destroyCairoContext (KeystoneCairoContext &context)
{
    if (context.cr)
...
bool
KeystoneScreen::checkDestination (unsigned int destX,
			      unsigned int destY)
{
    CompPoint point;
    CompSize  size;
...
void
KeystoneScreen::releaseMoveWindow ()
{
    CompWindow *window;
...
void
KeystoneScreen::computeTranslation (float &x,
				float &y)
{
...
void
KeystoneScreen::determineMovementAngle ()
{
    int   angle;
    float dx, dy;
...
bool
KeystoneScreen::moveViewport (int    x,
			  int    y,
			  Window moveWin)
{
    ...
    if (screen->otherGrabExist ("move", "switcher", "group-drag", "keystone", 0))
	return false;
    ...
    screen->handleCompizEvent ("keystone", "start_viewport_switch", o);

    if (!grabIndex)
	grabIndex = screen->pushGrab (screen->invisibleCursor (), "keystone");
...
void
KeystoneScreen::handleEvent (XEvent *event)
{
    switch (event->type) {
    case ClientMessage:
	if (event->xclient.message_type == Atoms::desktopViewport)
	{
	    ...
	    if (screen->otherGrabExist ("switcher", "keystone", 0))
		break;
...
void
KeystoneScreen::updateScreenEdgeRegions ()
{
...
void
KeystoneScreen::positionUpdate (const CompPoint &pos)
{
    if (edgeDrag)
	return;
...
void
KeystoneWindow::activate ()
{
    WALL_SCREEN (screen);

    if (window->placed () && !screen->otherGrabExist ("keystone", "switcher", 0))
    {
	int       dx, dy;
	CompPoint viewport;
...
void
KeystoneWindow::grabNotify (int          x,
			int          y,
			unsigned int width,
			unsigned int height)
{
    KeystoneScreen::get (screen)->toggleEdges (true);
    KeystoneScreen::get (screen)->edgeDrag = true;

    window->grabNotify (x, y, width, height);
}

void
KeystoneWindow::ungrabNotify ()
{
    KeystoneScreen::get (screen)->edgeDrag = false;

    window->ungrabNotify ();
}

void
KeystoneScreen::checkAmount (int 	      dx,
			 int 	      dy,
			 int          &amountX,
			 int          &amountY)
{
...
bool
KeystoneScreen::initiate (CompAction         *action,
...
bool
KeystoneScreen::terminate (CompAction         *action,
		       CompAction::State   state,
		       CompOption::Vector &options)
{
    if (showPreview)
...
bool
KeystoneScreen::initiateFlip (Direction         direction,
			  CompAction::State state)
{
    int dx, dy;
    int amountX, amountY;

    if (screen->otherGrabExist ("keystone", "move", "group-drag", 0))
	return false;
	...
	if (screen->otherGrabExist ("keystone", 0))
	    return false;
    }
...
inline void
keystoneDrawQuad (GLTexture::Matrix *matrix,
	      BOX               *box)
{
...
void
KeystoneScreen::drawCairoTextureOnScreen ()
{
...
void
KeystoneScreen::paint (CompOutput::ptrList& outputs,
		   unsigned int         mask)
{
    ...
    unsigned int              mask)
{
    ...
    GLMatrix sMatrix (matrix);
//    sMatrix.scale(0.95,0.95,1.0);
    glScreen->glPaintOutput (attrib, sMatrix, region, output, mask);
}

bool
KeystoneWindow::glPaint (const GLWindowPaintAttrib &attrib,
		     const GLMatrix            &matrix,
		     const CompRegion          &region,
		     unsigned int              mask)
{
//    WALL_SCREEN (screen);
    bool status = glWindow->glPaint (attrib, matrix, region, mask);
    return status;
}

void
KeystoneScreen::donePaint ()
{
    // Nothing to do
    cScreen->donePaint ();
}
...
void
KeystoneScreen::createCairoContexts (bool initial)
{
    int width, height;
...
void
KeystoneScreen::optionChanged (CompOption           *opt,
			   KeystoneOptions::Options num)
{
    switch(num) {
    case KeystoneOptions::OutlineColor:
	drawSwitcherBackground ();
	drawHighlight ();
	drawThumb ();
	break;

    case KeystoneOptions::EdgeRadius:
    case KeystoneOptions::BackgroundGradientBaseColor:
    case KeystoneOptions::BackgroundGradientHighlightColor:
    case KeystoneOptions::BackgroundGradientShadowColor:
	drawSwitcherBackground ();
	break;

    case KeystoneOptions::BorderWidth:
    case KeystoneOptions::PreviewScale:
	createCairoContexts (false);
	break;

    case KeystoneOptions::ThumbGradientBaseColor:
    case KeystoneOptions::ThumbGradientHighlightColor:
	drawThumb ();
	break;

    case KeystoneOptions::ThumbHighlightGradientBaseColor:
    case KeystoneOptions::ThumbHighlightGradientShadowColor:
	drawHighlight ();
	break;

    case KeystoneOptions::ArrowBaseColor:
    case KeystoneOptions::ArrowShadowColor:
	drawArrow ();
	break;

    case KeystoneOptions::NoSlideMatch:
...
bool
KeystoneScreen::setOptionForPlugin (const char        *plugin,
				const char        *name,
...
void
KeystoneScreen::matchExpHandlerChanged ()
{
...
void
KeystoneScreen::matchPropertyChanged (CompWindow *window)
{
    WALL_WINDOW (window);
    ...
    transform (NoTransformation)
{
    ...
    // HACK: we have to keep libcairo loaded even if keystone gets unloaded
    // to prevent crashes in XCloseDisplay
    dlopen ("libcairo.so.2", RTLD_LAZY);

    memset (&switcherContext, 0, sizeof (KeystoneCairoContext));
    memset (&thumbContext, 0, sizeof (KeystoneCairoContext));
    memset (&highlightContext, 0, sizeof (KeystoneCairoContext));
    memset (&arrowContext, 0, sizeof (KeystoneCairoContext));
    createCairoContexts (true);

#define setAction(action, dir, win) \
    optionSet##action##Initiate (boost::bind (&KeystoneScreen::initiate, this,   \
					      _1, _2, _3, dir, win));        \
    optionSet##action##Terminate (boost::bind (&KeystoneScreen::terminate, this, \
					       _1, _2, _3))

#define setFlipAction(action, dir) \
    optionSet##action##Initiate (boost::bind (&KeystoneScreen::initiateFlip, \
					      this, dir, _2))
...
#define setNotify(func) \
    ...
    poller.setCallback (boost::bind (&KeystoneScreen::positionUpdate, this,
				     _1));
}
